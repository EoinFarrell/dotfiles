...
**Root repo** — `~/Code/personal/dotfiles` (`$DOTFILES`). Bootstraps standalone: its `zsh/startup/init.sh` sets up the shell environment, exports `$DOTFILES_WD`, and sources the satellite repo if present. Defines shared machinery (sops helpers, `getLatestFromGit`, symlink conventions, docs format) that the satellite reuses rather than duplicates. Used on multiple personal machines (macOS + Debian).
